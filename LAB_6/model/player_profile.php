...
function editSellerDetails($userID, $email, $name, $phone, $region, $dob)
{
    $conn = db_conn();
    $query = "update player set email='$email', name='$name', phone='$phone', region='$region', dob='$dob' where id='$userID'";
    try {
        $conn->exec($query);
        $conn = null;
        return true;
...
retrieveOldPassword('99174686708989952');
